...
let optimizedField = (key, decode, json) =>
  if (
    ...
    | None => Obj.magic(getFailValueForOptimized())
    }
  } else {
    \"@@"(
      raise,
      DecodeError("Expected object, got " ++ _stringify(json)),
      /* Obj.magic(1001) */
    )
  }
  ...
  let rec inner = (decoders, errors) =>
    ...
    | list{} =>
      ...
      \"@@"(
        raise,
        DecodeError(
          j`All decoders given to oneOf failed. Here are all the errors: $revErrors. And the JSON being decoded: ` ++
          _stringify(json),
        ),
      )
    | list{decode, ...rest} =>
