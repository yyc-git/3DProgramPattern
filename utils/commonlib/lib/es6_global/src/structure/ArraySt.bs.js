...
import * as Log$Commonlib from "../log/Log.bs.js";
import * as Result$Commonlib from "./Result.bs.js";
import * as Contract$Commonlib from "../contract/Contract.bs.js";
import * as OptionSt$Commonlib from "./OptionSt.bs.js";
import * as PromiseSt$Commonlib from "./PromiseSt.bs.js";
import * as MutableHashMap$Commonlib from "./hash_map/MutableHashMap.bs.js";
...
function includesByFunc(arr, func) {
  return OptionSt$Commonlib.isSome(Js_array.find(func, arr));
}
...
function traverseResultM(arr, func) {
  if (arr.length === 0) {
    ...
    return Result$Commonlib.bind(Curry._1(func, _getExn(arr[0])), (function (h) {
                  return Result$Commonlib.bind(traverseResultM(Js_array.sliceFrom(1, arr), func), (function (t) {
                                return Result$Commonlib.succeed(Js_array.concat(t, [h]));
                              }));
                }));
  }
}
...
function traverseReducePromiseM(arr, func, param) {
  if (arr.length === 0) {
    ...
    return PromiseSt$Commonlib.bind(func(param, Caml_array.get(arr, 0)), (function (h) {
...
function traverseReduceResultM(arr, func, param) {
  if (arr.length === 0) {
    return Result$Commonlib.succeed(param);
  } else {
    return Result$Commonlib.bind(func(param, Caml_array.get(arr, 0)), (function (h) {
...
function deleteBySwap(arr, isDebug, index, lastIndex) {
  Contract$Commonlib.requireCheck((function (param) {
          var len = arr.length;
          Contract$Commonlib.test(Log$Commonlib.buildAssertMessage("lastIndex:" + lastIndex + " === arr.length:" + len, "not"), (function (param) {
                  return Contract$Commonlib.assertEqual(/* Int */0, arr.length - 1 | 0, lastIndex);
                }));
        }), isDebug);
...
function removeDuplicateItems(arr) {
  var resultArr = [];
  var map = MutableHashMap$Commonlib.createEmpty(undefined, undefined);
  for(var i = 0 ,i_finish = arr.length; i < i_finish; ++i){
    var item = arr[i];
    var key = item.toString();
    var match = MutableHashMap$Commonlib.get(map, key);
    if (match !== undefined) {
      
    } else {
      Js_array.push(item, resultArr);
      MutableHashMap$Commonlib.set(map, key, item);
    }
  }
  return resultArr;
}
