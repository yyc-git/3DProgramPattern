...
import * as Exception$Commonlib from "./Exception.bs.js";
...
var _buildErr = Exception$Commonlib.buildErr;

function failWith(x) {
  return {
          TAG: /* Error */1,
          _0: Exception$Commonlib.buildErr(x)
        };
}
...
function tryCatch(oneTrackFunc) {
  try {
    ...
    if (e.RE_EXN_ID === Js_exn.$$Error) {
      ...
      return {
              TAG: /* Error */1,
              _0: Exception$Commonlib.buildErr("unknown error: " + e)
            };
    }
  }
}
...
function getExn(result) {
  return handleFail(result, Exception$Commonlib.throwErr);
}
