...
export let exec: execType<worldState> = (worldState, { getStatesFunc }) => {
    ...
    return mostService.callFunc(() => {
        ...
        getAllGameObjects(worldState.ecsData.gameObjectManagerState).forEach(gameObject => {
            let material = getMaterialExn(worldState.ecsData.noLightMaterialComponentManagerState, gameObject)
            let transform = getTransformExn(worldState.ecsData.transformComponentManagerState, gameObject)

            let [count, program, color, modelMatrix] = getRenderData(material, transform, state.programMap, worldState.ecsData.noLightMaterialComponentManagerState, worldState.ecsData.transformComponentManagerState)

            render(gl, state.vbo.verticesVBO, state.vbo.indicesVBO, program, modelMatrix, color, count)
        })
