...
function _findFirstShaderLibExn(shaderLibName, shaderLibs) {
  return ArrayUtils$Chunk_handler.findFirstExn(shaderLibs, (function (item) {
...
function _getShaderLibsByBynamicBranch(resultShaderLibs, isPass, name, param) {
  ...
  if (JsonUtils$Chunk_handler.isJsonSerializedValueNone(dynamicBranchShaderLibNameOption)) {
    return resultShaderLibs;
  } else {
    return ArraySt$Commonlib.push(resultShaderLibs, _findFirstShaderLibExn(JsonUtils$Chunk_handler.getJsonSerializedValueExn(dynamicBranchShaderLibNameOption), param[1]));
  }
}
