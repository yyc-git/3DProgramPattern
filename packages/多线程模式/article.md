...
这三个部分的模块结构跟之前只支持单线程的领域模型的模块结构一样，只是其中的用户、门户、管道这几个部分中的模块不一样
具体不一样的地方如下：
...
1.创建其它线程的worker，启动它们的初始化
2.创建共享数据：RenderWorkderData的Buffer、PhysicsWorkerData的Buffer
3.发送数据到其它线程

它的具体流程如下：

1.主线程在“Create Worker Instance”Job中创建了渲染线程和物理线程的worker
这会执行它们的用户（RenderWorkerMain、PhysicsWorkerMain）的代码，从而运行它们的Init Pipeline，启动它们的初始化
2.主线程会执行三条并行的Job线
第一条Job线依次执行这些Job逻辑：
在“Create Render Data Buffer”Job中创建RenderWorkderData的Buffer、在“Create Physics Data Buffer”Job中创建PhysicsWorkerData的Buffer、在“Send Init Render Data”Job中向渲染线程发送初始化数据、在“Send Init Physics Data”Job中向物理线程发送初始化数据；
...
首先，我们看下主线程的Update Pipeline，它主要做了下面的事情：
1.更新共享数据RenderWorkderData的Buffer
2.启动渲染线程的渲染和物理线程的更新
3.发送数据到其它线程

它的具体流程如下：

1.因为组件可能发生了变化，所以在“Update Render Data Buffer”Job中更新了RenderWorkderData的Buffer中的组件数据；
...
然后在主线程进行同步时，再读取PhysicsWorkderData的Buffer，将其写到TransformComponent组件的Buffer中
...
然后，我们看下渲染线程中Client的代码；
然后，我们看下物理线程中Client的代码；
...
```ts
export let exec: ... = (worldState, { getStatesFunc, setStatesFunc }) => {
    ...
...
```ts
export let exec: ... = (worldState, { getStatesFunc }) => {
